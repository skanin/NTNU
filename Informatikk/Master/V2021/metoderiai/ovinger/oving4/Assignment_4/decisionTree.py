...
def run_titanic_example(columns):
    data = pd.read_csv('titanic/train.csv')
    data.fillna(data.mean(), inplace=True)
    dt: DecisionTree = DecisionTree(data, 'Survived', 1, 0, ['Age', 'Parch', 'SibSp'])
    tree = dt.decision_tree_learning(dt.data, columns, None)
    dt.draw_tree(tree, 'titanic')
    predictions = []
...
if __name__ == '__main__':
    run_titanic_example(['Sex', 'Embarked', 'Pclass', 'Parch', 'Age', 'SibSp', 'Survived'])